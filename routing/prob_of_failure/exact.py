...
def main():
    ...
    def route1_system_prob(p_link_bad, size):
        ...
        p_row_good = mp.power(p_link_good, size - 1)

        p_all_rows_good = mp.power(p_row_good, size)

        p_any_row_bad = 1 - p_all_rows_good

        p_system_bad = p_first_col_bad * p_any_row_bad
        return p_system_bad
    ...
    for size in sizes:
        for link_prob in link_probs:
            ...
            print(f'{size}x{size}, p_link_fail={link_prob:.3f}')
